...
#define ENCODER_ZERO_ANGLE_INIT                 (1)
#endif

#ifdef FakePodium_Rivu
#define Undervoltage_bus_centiV                 (1000)
#define CIRCUMFERENCE_1TURN_decimm              (2500.0)
#define Solenoid_Missing                        (1)
#define ENCODER_ZERO_ANGLE_INIT                 (1)
#endif
...
#define Solenoid_Missing                        (0)
#ifdef Rev6_spi_encoder
#define ENCODER_ZERO_ANGLE_INIT                 (1)
#endif
#ifdef Rev6_incremental_encoder
#define ENCODER_ZERO_ANGLE_INIT                 (1)
#endif
#endif
...
#define wt_centilb_to_engage_solenoid               (-500)
#define wt_lb_to_engage_solenoid                    (wt_centilb_to_engage_solenoid/100.0)
...
bool state_gpio11 = true; // only for PB2
bool state_gpio7 = true; // only for PB2
bool force_enabled_other_motor = false;
bool state_gpio50 = true; // only for PB2
bool state_gpio10 = true;
...
bool disable_inertia_friction_compensation = true;
      ...
      else
      {
          ...
          if(read_encoder_spiA_samples > 3)
          {
              if(read_encoder_spiA_samples > 65500)
              {
                  ...
                  //Encoder_count_offset = 5; // 5, 2053, hard code value got by init experiments for Rev6
              }
          ...
          if(EST_getState(obj->estHandle) == EST_State_Rs)   // if forcing alignment, using the Rs Recalculation, align the encoder angle with the rotor angle
          {
              ...
              read_encoder_spiA_samples_Rs_end = read_encoder_spiA_samples;
              if(read_encoder_spiA_samples_Rs_start == 0)
                  read_encoder_spiA_samples_Rs_start = read_encoder_spiA_samples;
          }
...
//   gMotorVars.BEMFAngle_iq = EST_getAngle_pu(obj->estHandle);
  ...
  if(undervoltage_shutdown_enable)
  {
      ...
      {
          gMotorVars.Flag_enableSys = false;
          HAL_setGpioLow(halHandle,GPIO_Number_44); // turn off relay
          force_enabled = false;
  ...
  if(setup_done && (!first_tare_ball_position_done))
  {
      if(ball_stop_down)
      {
          if(tare_ball_stop_counter == 5000) // 0.5 secs
          {
              ...
              //disable_inertia_friction_compensation = false; // make it true for normal operation
  ...
  if(first_tare_ball_position_done)
  {
      if(solenoid_engaged)
      {
          ...
          else
          {
              solenoid_pwm_duty_cycle = 1.0; // 12v ie 100% of 12v regulated voltage bus
              if(ball_stop_down)
                  wt_lb_set = wt_lb_to_engage_solenoid;
          }
      }
      else
      {
...
//      {
//          log_EncCount_QEP[log_array_pos] = 0;
//          log_EncCount_SPI[log_array_pos] = 0;
//          log_EncAngle_QEP[log_array_pos] = 0.0;
//          log_EncAngle_SPI[log_array_pos] = 0.0;
//          log_array_pos++;
//      }
//      else
//      {
...
//            {
//                log_EncCount_QEP[log_array_pos] = gMotorVars.EncCount;
//                log_EncCount_SPI[log_array_pos] = Encoder_count_multiturn;
//                log_EncAngle_QEP[log_array_pos] = _IQtoF(gMotorVars.EncAngle_iq);
//                log_EncAngle_SPI[log_array_pos] = _IQtoF(EncAngle_iq_1);
//                log_array_pos++;
//                log_counter = 1;
//            }
//            else
