...
uint16_t start_logging_decimm = 1450;
...
bool status_force_enabled_other_motor = false;
...
uint16_t tare_ball_stop_counter_1 = 0;
...
//float log_IqRef_pu[LOG_ARRAY_SIZE];
//float log_IqRef_pu_corr_inertia_accel[LOG_ARRAY_SIZE];
...
bool state_debug_input_pin = true; 
bool state_usb_to_serial_disconnected = true; 
bool serial_exchange_working_in_both_directions = true;
bool ball_stop_down_prev = false;
bool ball_stop_down = false;
uint16_t ball_stop_down_to_not_down = 0;
uint16_t ball_stop_not_down_to_down = 0;
...
void send_packet_over_ble_or_usb(HAL_Handle handle)
{
    ...
    case send_packet_over_ble_gather_data_step1 :
        ...
        ble_response_array[11] = Status_flags_other_motor & 0x00FF ;
#endif
    ...
    case send_packet_over_ble_gather_data_step4 :
        ...
        muscle_strain = SPIA_read_error_counter;//900000;
...
//      platform_force_balanced = false;
  ...
  status_force_enabled_other_motor = (bool)((advanced_mode_status_flags_other_motor & 0b0000000010000000) >> 7);
...
//   gMotorVars.BEMFAngle_iq = EST_getAngle_pu(obj->estHandle);
...
#if defined(Rev6)
  ball_stop_down_prev = ball_stop_down;
  ball_stop_down = HAL_readGpio(halHandle,(GPIO_Number_e)HAL_Gpio_ball_stop_down_IR_sensor);
  if(ball_stop_down_prev && (!ball_stop_down))
      ball_stop_down_to_not_down++;
  if((!ball_stop_down_prev) && ball_stop_down)
      ball_stop_not_down_to_down++;
  ...
  {
      if(force_enable_delay == 5000) // 0.5 sec delay
      {
          force_enabled = true;
  ...
  if(!setup_done)
  {
      ...
      {
          if(setup_done_counter == 1000) { // 100ms
              setup_done = true;
              setup_done_counter = 0;
          }
          else {
              setup_done_counter++;
          }
  ...
  {
      if(status_force_enabled_other_motor)
          solenoid_engaged = true;
  ...
  if(first_tare_ball_position_done)
  {
      ...
      if(ball_stop_down)
      {
          if(tare_ball_stop_counter_1 == 100) // 10ms
          {
              tare_ball_position = true;
              tare_ball_stop_counter_1 = 0;
          }
          else
          {
              tare_ball_stop_counter_1++;
          }
      }
      else
      {
  ...
  if(tare_ball_position)
  {
	  ...
	  //ball_accelx200_circular_buffer_zeroing_done = false;
	  //ball_velocityx10_circular_buffer_zeroing_done = false;
  }
  ...
  if(enable_inertia_compensation && enable_friction_compensation)
  {
      ...
      if(enable_inertia_compensation_force)
          IqRef_pu_corr = IqRef_pu + IqRef_pu_corr_inertia_accel;
      else
          IqRef_pu_corr = IqRef_pu;
  }
  else
  {
  ...
  if(IqRef_pu_corr < iq_var_max_limit)
      IqRef_pu_corr = iq_var_max_limit;
  ...
  {
      if(IqRef_pu_corr > iq_var_min_limit)
          IqRef_pu_corr = iq_var_min_limit;
  }

  // Set the Iq reference in per unit
  CTRL_setIq_ref_pu(ctrlHandle, IqRef_pu_corr);
...
//          log_IqRef_pu[log_array_pos] = 0.0;
//          log_IqRef_pu_corr_inertia_accel[log_array_pos] = 0.0;
//          log_array_pos++;
//      }
//      else
//      {
...
//                log_IqRef_pu[log_array_pos] = _IQtoF(IqRef_pu);
//                log_IqRef_pu_corr_inertia_accel[log_array_pos] = _IQtoF(IqRef_pu_corr_inertia_accel);
//                log_array_pos++;
//                log_counter = 1;
//            }
//            else
